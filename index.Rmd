...
patients_by_age <- patients %>% 
  ...
  filter(date >= max(date) - 365) %>% pivot_longer(-date, names_to = "age", values_to = "n")
patients_by_age <- newlycases %>% drop_na() %>% select(-detected, -detected_cum) %>% 
  pivot_longer(-date, names_to = "age", values_to = "n") %>% 
  mutate(age = case_when(age %in% c("age_00", "age_01_04", "age_05_09") ~ "10歳未満",
                          age == "age_10_19" ~ "10代",
                          age == "age_20_29" ~ "20代",
                          age == "age_30_39" ~ "30代",
                          age == "age_40_49" ~ "40代",
                          age == "age_50_59" ~ "50代",
                          age %in% c("age_60_64", "age_65_69") ~ "60代",
                          age == "age_70_79" ~ "70代",
                          age == "age_80_89" ~ "80代",
                          age == "age_90_" ~ "90代",
                          TRUE ~ NA_character_)) %>% drop_na() %>% 
  group_by(date, age) %>% summarise(n = sum(n)) %>% ungroup() %>% 
