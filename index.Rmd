...
by_region <- patients %>% 
  mutate(county = case_when(str_detect(address, "^福岡市") ~ "福岡市",
                            ...
                            str_detect(address, "下関市|熊本市|大分市|中津市|鳥栖市|長崎市|大阪市|佐世保市|京都市|広島市|裾野市|唐津市") ~ NA_character_,
...
map_data <- patients %>% filter(date >= max(date) - 6) %>% 
  ...
  mutate(region = case_when(str_detect(address, "^北九州市.*区") ~ str_remove(address, "北九州市"),
                            ...
                            str_detect(address, "[都道府県]$") ~ NA_character_,
                            str_detect(address, ".*県.*市") ~ NA_character_,
                            str_detect(address, "下関市|熊本市|大分市|中津市|鳥栖市|長崎市|大阪市|京都市|広島市|唐津市") ~ NA_character_,
