...
```{r}
detected %>% hchart("line", hcaes(x = date, y = value,)) %>% 
...
```{r}
untraceable %>% hchart("line", hcaes(x = date, y = value,)) %>% 
  hc_yAxis(title = FALSE,
           plotLines = list(
...
```{r}
positive_ratio %>% hchart("line", hcaes(x = date, y = value,)) %>% 
  hc_yAxis(title = FALSE,
           plotLines = list(
...
```{r}
total_patient %>% hchart("line", hcaes(x = date, y = value,)) %>% 
  hc_yAxis(title = FALSE,
           plotLines = list(
...
### 確保病床使用率（％）
```{r}
bed_occupancy %>% hchart("line", hcaes(x = date, y = value,)) %>% 
  hc_yAxis(title = FALSE,
           plotLines = list(
             list(value = 50, color = "#ff0000", width = 1),
             list(value = 20, color = "#ffa500", width = 1)
...
### 入院率（％）
```{r}
hospital_ratio %>% hchart("line", hcaes(x = date, y = value,)) %>% 
  hc_yAxis(title = FALSE,
           plotLines = list(
             list(value = 25, color = "#ff0000", width = 1),
...
under65_1st <- vaccination_rate %>% 
  filter(status == "65歳未満1回目") %>% last_value(ratio)
...
under65_2nd <- vaccination_rate %>% 
  filter(status == "65歳未満2回目") %>% last_value(ratio)
...
under65_3rd <- vaccination_rate %>% 
  filter(status == "65歳未満3回目") %>% last_value(ratio)
...
over65_1st <- vaccination_rate %>% 
  filter(status == "65歳以上1回目") %>% last_value(ratio)
...
over65_2nd <- vaccination_rate %>% 
  filter(status == "65歳以上2回目") %>% last_value(ratio)
...
over65_3rd <- vaccination_rate %>% 
  filter(status == "65歳以上3回目") %>% last_value(ratio)
...
newlycases %>%
  ...
  filter(date >= as.Date("2021-01-01")) -> tmp
...
tmp_data <- by_region %>% 
  ...
  mutate(value = value * 1e5 / population)
tmp_data %>% select(-population) %>% filter(date == max(date)) %>% 
  datatable(options = list(scrollY = "100%",
